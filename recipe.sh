...
if [ $stage -le 1 ]; then
    ...
    ops/tokenizer_train --config $dir/tokenizer.yaml --input $text --model $dir/tokenizer 2>$dir/log.tokenizer
fi
...
if [ $stage -le 5 ]; then
    ...
    ops/stt_error_rate --tokenizer char --ref $dir/ref.txt --hyp $dir/rec.txt $dir/RESULT 2>$dir/log.eval
fi
